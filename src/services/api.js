...
export const api = {
  ...
  sugerirCodigo: (categoriaId) => API.get(`/productos/sugerir-codigo?categoriaId=${categoriaId || ''}`).then(unwrap),
  verificarSku: (sku) => API.get(`/productos/sku/${sku}/exists`).then(unwrap),
  ...
  // Inventario
  getKardex: (productoId, page = 0) => API.get(`/kardex-nuevo/${productoId}?page=${page}&size=50`).then(unwrap),
  entradaStock: (data) => API.post('/inventario/entrada', data).then(unwrap),
  salidaStock: (data) => API.post('/inventario/salida', data).then(unwrap),
